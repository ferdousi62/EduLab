...
<a href="studentclass.php"  class="list-group-item list-group-item-action active" >Post</a>
    ...
    <a href="studentassignment.php" class="list-group-item list-group-item-action active" style="background:#2196f3;">Assignment </a>
...
<!--Show all Assignment-->
            ...
            <div class="col-md-8">
              ...
              <?php
             $sql= "SELECT * FROM assignment WHERE c_code='".$sc_code."' ORDER BY a_date DESC";
           ...
           <div class="inline"style="display: -webkit-box;">
            <div class="t_name" style="width: 78%;">  <?php echo $row['a_name']; ?> <br></div>
            
<?php
$aid= $row['a_id'];
$sql2= "SELECT * FROM assignment_submissions WHERE a_id='$aid' AND s_user_name='$susername' limit 1";

$result2 = $conn->query($sql2);
if($result2->num_rows > 0){

    while($row2 = $result2->fetch_array()){
        if($row2['a_marks'] != NULL){
        ?>
        <div class="marks" style=" font-family: 'Oswald', sans-serif; color: #075e2e;">Marks Obtained: <?php echo $row2['a_marks']; ?></div>
        <?php
        }
    }
}
...
?>
           ...
           // echo $currentdate;
            ...
            {?>
               ...
               <?php  
                //echo "Submission panel is closed<br>";
            }
            else
            {?>
                <div style=" width: -webkit-fill-available;color: #17682cd4;font-weight: 600;">
            ...
            <div style="  padding-right: 9px;  "> Submission Deadline: </div><?php echo date("Y-m-d h:iA",strtotime($row['a_duedate'])) ; ?>
              ...
              <a href="assignment_files/<?php echo $row['a_file']?>"target="_blank" ><div class="a_file"> <?php echo $row['a_file']; ?> </div></a><br>
              <?php
              $sql4= "SELECT * FROM assignment_submissions WHERE s_user_name= '$susername' AND a_id='".$row['a_id']."'";
              $result4=$conn->query($sql4);
              if($result4->num_rows > 0)
              {
                  ?>
                  <div class="alert alert-success alert-dismissible fade show">
    <strong>Success!</strong> Assignment Submitted
    
</div><?php
if($currentdate<$duedate){?>
<div class="submission_show"><button class="btn btn-primary mr-auto"><a href="student_assignment_submit.php?a_id=<?php echo $row["a_id"];?>"  style="font-family: 'Oswald';color: white;">Resubmit</a></button><br></div>
<?php
}
    ...
    <strong>Oh!</strong> You haven't submitted
             ...
             <?php }
              ...
              ?>
              <div class="submission_show"><button class="btn btn-primary mr-auto"><a href="student_assignment_submit.php?a_id=<?php echo $row["a_id"];?>"  
              style="font-family: 'Oswald';color: white;">
              Submit</a></button><br></div>
              <?php }?>
            </div>
            </div>
