...
if(isset($_REQUEST['changepass_btn'])){
    ...
    $sql= "SELECT * FROM t_registration WHERE user_name='$tusername'";
    $result = $conn->query($sql);
    if($result->num_rows == 1){
    ...
    if($tcurrentpass == $row['password']){
        ...
        $sql = "UPDATE t_registration set password= '$newpass' WHERE user_name='$tusername'";
        if($conn->query($sql)){
...
?>
    ...
    <div class="list-group"style="  font-family: 'Oswald', sans-serif;text-align: center;">
        ...
        <a href="changepassword.php" class="list-group-item list-group-item-action active" style="background:#2196f3;"> Change Password </a>
