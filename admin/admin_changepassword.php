...
if(isset($_REQUEST['changepass_btn'])){
    ...
    $tcurrentpass= $_REQUEST['currentpass'];
    $newpass = $_REQUEST['newpass'];

    $sql= "SELECT * FROM admin_tb WHERE email='$adminemail'";
    $result = $conn->query($sql);
    if($result->num_rows == 1){
        $row = $result->fetch_array();

       
    if($tcurrentpass == $row['password']){
        if(preg_match("#.*^(?=.{8,20})(?=.*[A-Za-z])(?=.*[0-9])(?=.*\W).*$#", $newpass)){

        $sql = "UPDATE admin_tb SET password= '$newpass' WHERE email='$adminemail'";
        if($conn->query($sql)){
            echo'<script>alert( "Password updated Successfully");</script>';
            echo "<script> location.href='admin_changepassword.php'</script>";
        }else{
...
?>
    ...
    <div class="list-group" style="  font-family: 'Oswald', sans-serif;text-align: center;">
        <a href="adminDashboard.php"  class="list-group-item list-group-item-action active" style="background:#1d0942" >Dashboard</a>
        ...
        <a href="admin_changepassword.php" class="list-group-item list-group-item-action active"style="background:#9c27b0;"> Change Password </a>
...
<form method="post" action="admin_changepassword.php">
        ...
        <button type="submit" style="background: #9c27b0; color: aliceblue;font-family: 'Oswald';" name="changepass_btn" class="btn ">Change Password</button>
        </div>
