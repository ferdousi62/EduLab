...
if(isset($_SESSION['lecture_id'])){
    ...
    {
        $_SESSION['lecture_id']=  $_REQUEST["file_id"];
        $lecture_id= $_SESSION['lecture_id'];
    }
...
if(isset($_REQUEST['sc_code']) && isset($_REQUEST['scourse_name'])){
    ...
    if(preg_match('/^[ ]*$/i', $comment)){
        ...
        echo "<script> location.href='student_discussion_panel.php'; </script>";
    }else{
    
    $sql2 = "INSERT INTO discussion_panel (lecture_id,user_name,full_name,comment) VALUES ('$lecture_id','$susername','$sfullname','$comment')";
...
<?php
                      if($result->num_rows > 0){
                        while($row = $result->fetch_array()){
                             ?>
<div style="font-size: large;"><?php echo $row['lecture_date']?></div>
...
<form action="student_discussion_panel.php" method="POST" >
            <textarea rows="3" class="form-control"  name="comment" placeholder="Type your note here." required></textarea>
...
<?php  if($row['user_name']== $susername){ ?>
<!--Dropdown menu-->
<div  id="test">
...
<div class="dropdown-menu">
 <div class="dropdown-item">   <a href="delete_comment_student.php?query_id=<?php echo $row["query_id"]; ?>"><button>Delete</button></a></div>
</div>
...
<!--Show all Assignment-->
         ...
         <div class="demo-content">
              ...
              <div class="t_name test6" style="font-weight: 500;"><?php echo $sfullname;?></div>
